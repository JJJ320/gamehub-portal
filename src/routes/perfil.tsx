...
export const Route = createFileRoute("/perfil")({
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Meu perfil — GameHub" },
      {
...
const avatarSchema = z.union([
  z.literal(""),
  z.string().trim().url({ message: "Informe uma URL válida de imagem" }).max(500),
]);
...
function ProfilePage() {
  ...
  useEffect(() => {
    if (!loading && !user) navigate({ to: "/auth", search: { redirect: "/perfil" }, replace: true });
  ...
  const initials = (profile?.display_name ?? user.email ?? "G").slice(0, 2).toUpperCase();
  ...
  const save = async (e: React.FormEvent) => {
    ...
    if (!nameResult.success) next["displayName"] = nameResult.error.issues[0]!.message;
    if (!avatarResult.success) next["avatarUrl"] = avatarResult.error.issues[0]!.message;
    ...
    const { error } = await supabase.from("profiles").upsert({
      ...
      avatar_url: avatarResult.data ? avatarResult.data : null,
    });
    setSaving(false);
    if (error) {
      setStatus({ kind: "error", text: "Não foi possível salvar. Tente novamente." });
      return;
    }
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
        ...
        <header className="mt-4 flex flex-col gap-4 rounded-2xl border border-border bg-card p-5 shadow-card sm:flex-row sm:items-center sm:p-6">
          ...
          <Button variant="outline" className="sm:ml-auto" onClick={handleSignOut}>
